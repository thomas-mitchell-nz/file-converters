...
def convert_avi_to_mp4_with_metadata(source_folder, target_folder=None):
    ...
    for filename in os.listdir(source_folder):
        if filename.endswith(".avi") or filename.endswith(".AVI"): 
            ...
            try:
                ...
                # Convert .avi to .mp4 and set creation_time if available
                if creation_time:
                    ffmpeg.input(source_path).output(target_path, vcodec='libx264', crf=22, pix_fmt='yuv420p', metadata='creation_time=' + creation_time).run(overwrite_output=True)
                else:
                    ffmpeg.input(source_path).output(target_path, vcodec='libx264', crf=22, pix_fmt='yuv420p').run(overwrite_output=True)
...
if __name__ == "__main__":
    source_folder = 'file-converters/avi' 
    target_folder = 'file-converters/mp4'
    convert_avi_to_mp4_with_metadata(source_folder, target_folder)
